...
class ZooAPIClient {
  ...
  async getAliance(hash, accountIndex) {
    ...
    try {
      ...
      this.log(`Lỗi khi get Aliance: ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
  ...
  async setQuiz(hash, accountIndex, questKey, result) {
    ...
    try {
      ...
      this.log(`Lỗi khi kiểm tra quiz "${questKey}": ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
  ...
  async claimQuiz(hash, accountIndex, questKey) {
    ...
    try {
      ...
      this.log(`Lỗi khi claim quiz "${questKey}": ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
  ...
  async completeAllQuests(initData, accountIndex) {
    try {
      ...
      for (const quiz of quizsAvaliable) {
        this.log(`Bắt đầu quiz ${quiz.title}...`, "info");
        ...
        if (claimResult.success === true) {
          this.log(`Hoàn thành quiz ${quiz.key} | "${quiz.title}", nhận ${quiz.reward} phần thưởng.`, "success");
        } else if (claimResult.error === "already rewarded") {
          this.log(`quiz ${quiz.key} "${quiz.title}" đã được hoàn thành trước đó.`, "warning");
        } else {
          this.log(`Không thể hoàn thành  hoặc cần làm bằng tay quiz ${quiz.key} | "${quiz.title}": ${claimResult.error}`, "warning");
        }
        await new Promise((resolve) => setTimeout(resolve, 1000));
      }
    } catch (error) {
      this.log(`Lỗi khi lấy danh sách quiz: ${error.message}`, "error");
    }
  }
  ...
  async runAccount() {
    ...
    try {
      ...
      if (loginResult.success) {
        ...
        this.log(`Đăng nhập thất bại: ${loginResult.error}`, "warning");
      }
