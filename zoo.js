...
class ZooAPIClient {
  ...
  async getAliance(hash, accountIndex) {
    ...
    try {
      ...
      this.log(`Error while getting Aliance: ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
  ...
  async joinAliance(hash, accountIndex, id) {
    ...
    try {
      ...
      if (response.status === 200 && response.data.success) {
        ...
        this.log(`Join aliance ${alliance.name} | Level: ${alliance.level} success!`, "success");
        return response.data;
      }
    } catch (error) {
  ...
  async claimQuiz(hash, accountIndex, questKey) {
    ...
    try {
      ...
      this.log(`Error when claim quiz "${questKey}": ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
    ...
    try {
      ...
      for (const quest of quests) {
        ...
        if (claimResult.success === true) {
          this.log(`Complete the mission ${quest.key} | "${quest.title}", nhận ${quest.reward} award.`, "success");
      ...
      for (const quiz of quizsAvaliable) {
        ...
        if (claimResult.success === true) {
          this.log(`Complete the quiz ${quiz.key} | "${quiz.title}", nhận ${quiz.reward} award.`, "success");
        } else if (claimResult.error === "already rewarded") {
          this.log(`quiz ${quiz.key} "${quiz.title}" has been completed before.`, "warning");
        } else {
          this.log(`Unable to complete or need to do the quiz manually ${quiz.key} | "${quiz.title}": ${claimResult.error}`, "warning");
        }
        await new Promise((resolve) => setTimeout(resolve, 1000));
      }
    } catch (error) {
          ...
          if (dbAnimal) {
            ...
            if (nextLevelData && hero.coins >= nextLevelData.price) {
              ...
              try {
                ...
                if (response.status === 200 && response.data.success) {
                  this.log(`Upgrade ${dbAnimal.title} success level ${nextLevel}`, "success");
                }
              } catch (error) {
  ...
  async main() {
    this.log(`Tool developed by tele group Airdrop Hunter Super Speed (https://t.me/AirdropScript6)`, "warning");
    try {
      ...
      while (true) {
                  ...
                  for (let day = 1; day <= 16; day++) {
                    if (dailyRewards[day] === "canTake") {
                      ...
                      if (claimResult.success) {
                        this.log("Daily roll call success!", "success");
                      }
                      break;
                    }
                  }
        ...
        if (firstAccountFeedTime) {
          ...
          this.log(`Use default timeout ${settings.TIME_SLEEP} minute`, "info");
        }
